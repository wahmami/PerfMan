...
def load_auth_secrets():
    try:
        ...
        st.error("Missing [auth] section in .streamlit/secrets.toml. Create .streamlit/secrets.toml with an [auth] section containing usernames, names and passwords.")
        st.stop()
    usernames = auth.get("usernames", [])
    names = auth.get("names", [])
    passwords = auth.get("passwords", [])
    if not (usernames and names and passwords):
        st.error("Incomplete auth secrets: ensure 'usernames', 'names' and 'passwords' are present under [auth] in .streamlit/secrets.toml.")
        st.stop()
    return usernames, names, passwords

# --- Advanced Authentication Setup ---
usernames, names, passwords = load_auth_secrets()
